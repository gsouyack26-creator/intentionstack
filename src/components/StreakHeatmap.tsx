...
function getCellColor(count: number, hasEntry: boolean): string {
...
export const StreakHeatmap: React.FC<StreakHeatmapProps> = ({ entries }) => {
    ...
    <div className="flex flex-col gap-2">
      <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
        {days.map((day, idx) => {
          ...
          const color = getCellColor(count, !!entry);
      ...
      <div className="flex items-center gap-2 justify-end mt-1">
        ...
        {[0, 1, 2, 3].map(n => (
          <div key={n} className="w-3 h-3 rounded-sm" style={{ backgroundColor: getCellColor(n, n > 0) }} />
        ))}
